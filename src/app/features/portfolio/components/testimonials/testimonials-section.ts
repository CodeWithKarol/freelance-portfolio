          ...
          <div class="-mt-8 sm:-mx-4 sm:columns-2 lg:columns-3 sm:gap-4">
            @for (test of testimonials(); track test.id) {
              <div class="pt-8 sm:inline-block sm:w-full sm:px-4">
                <figure class="rounded-2xl bg-white dark:bg-slate-900 p-8 text-sm leading-6 shadow-sm ring-1 ring-slate-900/5 dark:ring-slate-800 relative group hover:-translate-y-1 transition-transform duration-300 ease-in-out">
                  
                  <!-- Quote Icon Backdrop (decorative) -->
                   <div class="absolute top-6 right-8 text-slate-100 dark:text-slate-800">
                     <svg class="h-12 w-12 transform group-hover:scale-110 transition-transform duration-500" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  <figcaption class="mt-6 flex items-center gap-x-4">
                    ...
                    <div class="h-10 w-10 flex-none rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center ring-1 ring-slate-200 dark:ring-slate-700">
                         <span class="font-bold text-slate-500 dark:text-slate-400 text-xs">{{ test.name.charAt(0) }}</span>
                    </div>
                    <div>
                      <div class="font-semibold text-slate-900 dark:text-white">{{ test.name }}</div>
                      <div class="text-slate-600 dark:text-slate-400 text-xs">{{ test.role }} @ {{ test.company }}</div>
                    </div>
                  </figcaption>
                </figure>
              </div>
